...
class Move {
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            Move result = move(move0.col0(), move0.row0(),
                    move1.col0(), move1.row0()
                    );
            return move(result, move1);
        }
        if (move0.jumpTail() == null) {
            Move help1 = move(move0.col0(), move0.row0(),
    ...
    boolean isLeftMove() {
        ...
        boolean colDiff = col0() > col1();
        boolean rowDiff = row0() == row1();
        return colDiff && rowDiff;
    }

    /** Return true iff this is a vertical move. */
    ...
    private void toString(Formatter out) {
        out.format("%s%s%s%s%s" , col0(), row0(), "-", col1(), row1());
        Move cur = jumpTail();
        while (cur != null) {
            out.format("%s%s%s", "-", cur.col1(), cur.row1());
            cur = cur.jumpTail();
        }
    }
